...
import { Newspaper, Edit2, Eye, EyeOff } from "lucide-react";
import TopBar from "@/components/reusable_components/topBar";
import { useTranslation } from "react-i18next";
import { ThemeContext } from "@/app/contexts/ThemeContext";
import { apiClient, User, Subscription } from "../../../lib/api";
...
const SettingsPage = () => {
  ...
  if (!context) throw new Error("SettingsPage must be used inside ThemeProvider");
  ...
  const addSubscription = async () => {
    const trimmedKey = newSubscription.trim();
    if (!trimmedKey) return;
    setAddingSubscription(true);
    try {
      const sub = await apiClient.subscribe(trimmedKey);
      if (!subscriptions.some((s) => s.source_slug === sub.source_slug)) {
  ...
  const removeSubscription = async (slug: string) => {
    try {
      await apiClient.unsubscribe(slug);
      setSubscriptions((subs) => subs.filter((s) => s.source_slug !== slug));
    } catch (err) {
    ...
    <>
      ...
      <div
        className={`flex flex-col md:flex-row min-h-screen transition-colors duration-300 ${
          theme === "dark" ? "bg-[#1A1A1A] text-gray-100" : "bg-gray-50 text-gray-900"
        }`}
      >
        <div className="flex-1 pt-6 md:pt-10 px-4 sm:px-6 lg:px-8 flex justify-center">
          <div className="w-full max-w-4xl">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-8 gap-4 md:gap-0">
              <h1 className="text-2xl sm:text-3xl font-semibold">{t("settings.title")}</h1>
              <button onClick={handleSave} className={saveBtnClass} disabled={saving}>
            ...
            {activeTab === "customization" && (
              <section className={sectionClass}>
                <h2 className="text-lg font-medium mb-4">{t("settings.tabs.customization")}</h2>
                <div className="mb-4">
            ...
            {activeTab === "account" && (
              <section className={sectionClass}>
                <h2 className="text-xl sm:text-2xl font-semibold mb-4">{t("settings.account.title")}</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                  ...
                  <div className="relative">
                    ...
                    <div className="relative">
                      <input
                        ...
                        className={`${inputClass} pr-10 ${editingFullName ? "text-gray-900" : "text-gray-400"}`}
                  ...
                  <div className="relative sm:col-span-2">
                    ...
                    <div className="relative">
                      <input
                        type={editingPassword ? (passwordVisible ? "text" : "password") : "text"}
                        value={editingPassword ? password : "●●●●●●●"}
                        onChange={(e) => setPassword(e.target.value)}
                        placeholder={t("settings.account.placeholders.password")}
                        className={`${inputClass} pr-10 ${editingPassword ? "text-gray-900" : "text-gray-400"}`}
            ...
            {activeTab === "categories" && (
              <section className={sectionClass}>
                <h2 className="text-lg font-medium mb-4">{t("settings.tabs.categories")}</h2>
                <div className="flex flex-wrap gap-2 mb-3">
                  {tags.map((tag) => (
                    <span key={tag} className="flex items-center gap-2 bg-gray-100 text-gray-800 px-3 py-1.5 rounded-full text-sm font-medium">
                      {tag}
            ...
            {activeTab === "subscriptions" && (
              <section className={sectionClass}>
                <h2 className="text-lg font-medium mb-4">{t("settings.tabs.subscriptions")}</h2>
                <div className="space-y-3 mb-3">
                  {subscriptions.map((sub) => (
                    <div key={sub.source_slug} className="flex flex-col sm:flex-row sm:justify-between items-start sm:items-center border border-gray-200 rounded-xl px-4 py-3 gap-2 sm:gap-0">
                      <div className="flex items-center gap-3">
                        <Newspaper className="w-5 h-5" />
                        <span className="text-gray-800 font-medium">{sub.source_name}</span>
                      </div>
                      <button
                        onClick={() => removeSubscription(sub.source_slug)}
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                  ...
                  <button
                    ...
                    disabled={addingSubscription || !newSubscription.trim()}
                    className="bg-[#0B66FF] text-white px-4 py-2 rounded-lg hover:bg-[#EAF2FF] hover:text-black transition w-full sm:w-auto"
                  >
                    + Add
                  </button>
